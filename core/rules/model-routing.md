...
The "Theoretical; not yet measured" entries are honest acknowledgements. Five task classes are now empirically tested (full-document drafting in case-study-01, idea expansion in case-study-03, idea triage in case-study-04, codemap drafts in case-study-05, summarization in case-study-06); the remaining strong-fit rows are v0.2 measurement targets. A consistent pattern across the four measured local-tier strong-fit rows: structured-output tasks at this hardware tier produce zero hallucinations and high content quality, but the model's reasoning behavior depends on context size and budget. Rule of thumb from case-study-05 + case-study-06: set `max_tokens` just high enough to keep reasoning channeled, and set `lm_studio_context` just large enough for the prompt — not larger.
